$(document).ready(function () {
  ...
  // $('#level_code').html('<option value="">--กรุณาเลือกระดับ / ชั้น--</option>');
  ...
  $("#level_code").change(function () {
    ...
    if (level_code) {
      $.post(
        "../../phtools/PhDropDown/get_standard_qualification",
        {
          tier1_code,
          tier2_code,
          tier3_id,
...
function get_dataforedit() {
	if ($("#action").val() == "update" || $("#action").val() == "copy") {
		...
		$.ajax({
			url: "../../simulation/SimulationTools/get_template",
			method: "POST",
			data: {
...
function fileManager(tamplet_id){
	...
	$.ajax({
        url: "../../simulation/SimulationTools/file_manager",
        method: "POST",
        data: {
...
function sendtemplate_approve() {
	$.ajax({
		method: "POST",
		url: "../../simulation/SimulationTools/sendtemplate_approve",
		data: {
			template_id: $("#template_id").val(),
		},
		dataType: "JSON",
