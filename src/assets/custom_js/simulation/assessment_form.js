...
$("#btn_save").click(function () {
  ...
  $.ajax({
	method: "POST",
	url: "../../demonstration/DemonstrationTools/insert_assessment_applicant",
	data: {
	...
	success: function (response) {
	  if (response.status == 1) {
		...
		//อัพโหลดไฟล์ และ อัพชื่อไฟล์
		uploadFileAssessment(fileData, nameFile, app_id, response.assessment_detail_id)
	  }
	},
  });
})
...
function checkForm() {
	return $("input[type=number]").filter(function () {
	...
	Swal.fire({
		...
		if (result.isConfirmed) {
			...
			$(".file-data").each(function () {
				var uploadFile = document.getElementById($(this).attr("id")).files;
				fileData.push(uploadFile);
				nameFileData.push(uploadFile[0].name);
			});

			console.log(nameFile);
			console.log(nameFileData);
			...
			$.ajax({
				method: "POST",
				url: "../../simulation/SimulationTools/insert_assessment_applicant",
				data: {
					dt: JSON.stringify($("form").serializeJSON()),
					exam_schedule_id: $("#exam_schedule_id").val(),
					app_id: app_id,
					name_file_data: nameFileData,
					name_file: nameFile,
					tool_type: "4",
				},
				dataType: "JSON",
	...
	for (u = 0; u < fileData.length; u++) {
		...
		for (i = 0; i < file.length; i++) {
			...
			if (file[i] != null) {
				...
				var url = "../../simulation/SimulationTools/saveFileAssessment";
...
function saveDetailFile(data, assessment_id) {
	$.ajax({
		method: "POST",
		url: "../../simulation/SimulationTools/saveDetailFile",
		data: {
			data: data,
