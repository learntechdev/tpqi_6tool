...
function get_uoc_list(occ_level_id) {
	$.ajax({
		url: "../../tools/Thirdparty/fetch_uoc",
		method: "POST",
		data: {
			occ_level_id: occ_level_id,
...
$("#files").change(function () {
	if ($("#files").val() != "") {
		$.each($("#files").prop("files"), function (k, v) {
			...
			if ($.inArray(ext, ["pdf", "doc", "docx", "mp4"]) == -1) {
				sweet_alert(
					"<strong>กรุณาเลือกประเภทไฟล์เป็น pdf mp4 และ doc เท่านั้น!!!</strong>"
				);
		...
		success: function (response) {
			if (response == 1) {
				...
				window.location.replace(
					"../../exam_library/Examlibrary/index?tool_type=7&contract_no=" +
						$("#contract_no").val()
				);
			}
		},
