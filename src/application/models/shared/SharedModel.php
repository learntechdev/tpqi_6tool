...
class SharedModel extends CI_Model
{
	...
	public function fmanagerlayout($view, $data)
    {
        $this->UIModel->fmheader();
        $this->load->view($view, $data);
        $this->UIModel->fmfooter();
    }
	...
	public function get_username($user_citizen_id){
    $citizen_id_array = array_map('intval', explode(',', $user_citizen_id));
    $result = []; 

    if (is_array($citizen_id_array)) {
        for ($i = 0; $i < count($citizen_id_array); $i++) {
            $citizen_id = $citizen_id_array[$i];
            $sql = "SELECT CONCAT(prefix, first_name, ' ', last_name) AS fullname FROM user_login 
                    WHERE citizen_id = '" . $citizen_id . "'";

            // Fetch result
            $result_value = $this->BaseModel->get_one_field($sql);

            // Handle case where result is an array
            if (is_array($result_value)) {
                // Extract a specific value if needed (e.g., assume 'fullname' is the key)
                $result[] = isset($result_value['fullname']) ? $result_value['fullname'] : '';
            } else {
                // Assume result is a scalar value (string)
                $result[] = $result_value;
            }
        }
    }
	...
	public function upload_file($formdata)
    {
		$uploaded_files = $formdata["fileName"];
		$files_length = count($uploaded_files);
		if($files_length >0){
		foreach ($uploaded_files as $index => $file) {
			$filename = htmlspecialchars($file);
        $data = [
			"tier1_title" => $formdata["qualification2"],
			"tier2_title" => $formdata["branch2"],
			"tier3_title" => $formdata["occupation2"],
			"level_name" => $formdata["level2"],
			"interview" => $formdata["interview"],
			"demonstrate_work_performance" => $formdata["show"],
			"portfolio" => $formdata["file"],
			"third_party_assessment" => $formdata["evaluate"],
			"simulate_the_situation" => $formdata["mock"],
			"observe_on-site" => $formdata["observe"],
			"RESK" => $formdata["resk"],
			"file_name" => $filename,
			"file_type" => "pdf",
			"user_uploaded" => $_SESSION['name'],
        ...
        return $this->BaseModel->insert("req_docs_forexam", $data);
		}
		}
    }
