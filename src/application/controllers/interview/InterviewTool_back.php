...
class InterviewTool extends CI_Controller
{
	...
	public function index()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'GET') {
            // Handle GET request for fetching data, like previewing template, fetching UOC, etc.
            $this->handleGetRequest();
        } elseif ($_SERVER['REQUEST_METHOD'] === 'POST') {
            // Handle POST request for creating or inserting data, like saving template, assessment, etc.
    ...
    {
        // Example for previewing a template
        if (isset($_GET['template_id'])) {
            $this->preview();
        }
        // Example for fetching UOC or EOC data
        else if (isset($_GET['occ_level_id']) && isset($_GET['template_id']) && isset($_GET['asm_tool'])) {
            $this->get_uoc();
        } else {
    ...
    {
        // Check if it's a request to create or update a template
        if (isset($_POST['action'])) {
            if ($_POST['action'] === 'create' || $_POST['action'] === 'update') {
                $this->insert();  // This method handles template creation and updates
            } else {
                echo "Invalid action for POST request.";
            }
        } else {
