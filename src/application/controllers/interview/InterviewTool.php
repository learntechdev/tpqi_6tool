...
class InterviewTool extends CI_Controller
{
	...
	//	$json = str_replace(array("\t", "\n"), "", $_POST["dt"]);
     //   $data = json_decode($json);
		...
		if (json_last_error() !== JSON_ERROR_NONE) { die("JSON decode error: " . json_last_error_msg()); }
		if (!isset($decodedData->dt)) { die("Error: 'dt' key not found in the JSON payload."); } $data = $decodedData->dt;
		$tool_type = isset($decodedData->tool_type) ? $decodedData->tool_type : null;  
		
	
        if ($data->action == "create") {
            if ($data->template_id == "") {
            //    $template_id = $this->MasterDataModel->gen_asm_template($_POST["tool_type"]);
				$template_id = $this->MasterDataModel->gen_asm_template($tool_type);
    ...
    {
        $str_occ_level_id = $action == "copy" ? $data->txt_occ_level : $data->occ_level_id1;
    ...
    //    $str_criteria_type_byexamier = $data->criteria_used_byexamier1 == 0 ? "" : $data->criteria_type_byexamier1;
		$str_criteria_type_byexamier = $data->criteria_used_byexamier1 == 0 ? "" : $data->criteria_used_byexamier2;
        ...
        $data_arr = [
            ...
            "criteria_used_byexamier" => $data->criteria_used_byexamier1,
