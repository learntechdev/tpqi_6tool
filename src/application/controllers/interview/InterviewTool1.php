...
class InterviewTool extends CI_Controller
{
    ...
    {
    $data["uoc"] = $this->MasterDataModel->fetch_uoc($_GET["keyword"]);
    $data["eoc"] = $this->MasterDataModel->fetch_eoc($_GET["keyword"]);
    $this->load->view("interview/show_uoc", $data);
    }

    public function fetch_eoc()
    {
    $action = $_GET["action"];
    if ($action == "load") {
    $data["eoc"] = $this->MasterDataModel->fetch_eoc($_GET["keyword"]);
    } else {
    $data["eoc"] = $this->MasterDataModel->fetch_eoc1($_GET["keyword"]);
    }
    // $data["template_id"] = null;
    $this->load->view("interview/show_eoc", $data);
    }
     */
