...
class Thirdparty extends CI_Controller
{
    public function __construct()
    {
        ...
        $this->load->model("thirdparty/ThirdpartyToolsModel");
    ...
    public function create()
    {
        ...
        $this->SharedModel->layouts("thirdparty/form", $data);
    }
...
//        $str_criteria_type_byexamier = $data->criteria_used_byexamier == 0 ? "" : $data->criteria_type_byexamier;
        ...
        if ($rs) {
            if ($str_exam_type == "2" && $str_template_type == "1") { //ตาม uoc & หัวข้อหลัก
                $rs_tp_detail = $this->SharedModel->insert_chklist_uoc_maintopic("7", $template_id, $data->uocchklist);
            }
        }

        $this->SharedModel->insert_user_log("created", "thirdparty", $template_id, '', '');
        return 1;
    }
    ...
    private function update_tp($template_id, $data, $action)
    {
        ...
        $this->SharedModel->insert_user_log("updated", "thirdparty", $template_id, '', '');
        return 1;
    }
    ...
    public function exam_preview()
    {
        ...
        if ($exam_type == "2" && $template_type == "1") {
            ...
            $data['tool_typename'] = "ประเมินด้วยบุคคลที่สาม";
    ...
    public function assessment_create()
    {
        ...
        $this->SharedModel->layouts("thirdparty/assessment/form", $data);
    }
    ...
    public function insert_assessment_applicant()
    {
        ...
        if ($data->action == "create") {
            ...
            $rs = $this->ThirdpartyToolsModel->insert_assessment($assessment, $data->detail, $tool_type);
        }
        echo json_encode($rs);
    }
    ...
    public function saveFileAssessment()
    {
        ...
        if (!@mkdir('thirdparty_assessment_flie/' . $_POST['app_id'], 0, true)) {
        ...
        $path = "thirdparty_assessment_flie/" . $_POST['app_id'] . "/";
        chmod($path, 0777);
    ...
    public function saveDetailFile()
    {
        ...
        $rs = $this->ThirdpartyToolsModel->save_assessment_detail_file($_POST['data'], $_POST['assessment_id']);
        echo json_encode($rs);
    }
    ...
    //    $data["active_title"] = array("active_title" => "ตัวจัดการไฟล์");
        $this->SharedModel->fmanagerlayout("thirdparty/filemanager", $data);
