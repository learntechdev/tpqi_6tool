...
class Observation extends CI_Controller
{
    ...
    public function create()
    {
        ...
        $this->SharedModel->layouts("observation/form", $data);
    }
...
//        $str_criteria_type_byexamier = $data->criteria_used_byexamier == 0 ? "" : $data->criteria_type_byexamier;
        ...
        if ($rs) {
            if ($str_exam_type == "2" && $str_template_type == "1") { //ตาม uoc & หัวข้อหลัก
                $rs_tp_detail = $this->SharedModel->insert_chklist_uoc_maintopic("6", $template_id, $data->uocchklist);
            }
        }

        $this->SharedModel->insert_user_log("created", "observation", $template_id, '', '');
        return 1;
    }
    ...
    private function update_tp($template_id, $data, $action)
    {
        ...
        $this->SharedModel->insert_user_log("updated", "observation", $template_id, '', '');
        return 1;
    }
    ...
    public function exam_preview()
    {
        ...
        if ($exam_type == "2" && $template_type == "1") {
            $docFileName = "preview/uoc_checklist_maintopic_html";
            $data['tool_typename'] = "สังเกตการณ์ในหน้างานจริง";
    ...
    public function assessment_create()
    {
        ...
        // $data["template"] = $this->SharedModel->get_template_forexam($_POST['template_id'], $_POST['exam_schedule_id']);
        $data["template"] = $this->SharedModel->get_template_forass($_POST['exam_schedule_id'], $_POST["tool_type"], $_POST['occ_level_id']);

        $data['template_detail'] = $this->SharedModel->get_chklist_uoc($_POST['template_id']);

        $this->SharedModel->layouts("observation/assessment/form", $data);
    }
    ...
    public function insert_assessment_applicant()
    {
        ...
        if ($data->action == "create") {
            ...
            $rs = $this->ObservationToolsModel->insert_assessment($assessment, $data->detail, $tool_type);
        } else {
        }
        echo json_encode($rs);
    }
    ...
    public function saveFileAssessment()
    {
        ...
        if (!@mkdir('observation_assessment_flie/' . $_POST['app_id'], 0, true)) {
        ...
        $path = "observation_assessment_flie/" . $_POST['app_id'] . "/";
        chmod($path, 0777);
    ...
    public function saveDetailFile()
    {
        ...
        $rs = $this->ObservationToolsModel->save_assessment_detail_file($_POST['data'], $_POST['assessment_id']);
        echo json_encode($rs);
    }
    ...
    //    $data["active_title"] = array("active_title" => "ตัวจัดการไฟล์");
        $this->SharedModel->fmanagerlayout("observation/filemanager", $data);
