            ...
            <tr>
                ...
                <td class="text-center">
                    ...
                    <button type="button" class="btn btn-primary btn-bg" id="btn_approve" name="btn_approve"
                        onClick="show_criteria('<?= $v['occ_level_id'] ?>','<?= $v['occ_level_name'] ?>','<?= $v['place'] ?>','<?= $this->BaseModel->dateToThai($v['start_date'], false) ?>','<?= $v['exam_schedule_id'] ?>','<?= $v['status'] ?>','<?= $v['asm_tool_type'] ?>','<?= $v['exam_template_id'] ?>')">กำหนดเกณฑ์
                    </button>
                    <?php }
                            if ($v["status"] == 5) { ?>
                    ...
                    <?php }
                            if ($v["status"] == 1 && $v["exam_template_id"] != "") { ?>
...
<div class=" col-md-12" sytle="display:none">
    <?php
    require_once dirname(__FILE__) . "../../../criteria_asm/form_define_criteria.php"; ?>
</div>