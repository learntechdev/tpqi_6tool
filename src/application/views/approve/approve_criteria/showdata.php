    ...
    foreach ($dataList["result"] as $key => $v) {
        ...
        <tbody>
            <tr>
                ...
                <td class="text-center">
                    <?=$this->BaseModel->dateToThai($v['exam_date'], false)?>
                </td>
                <td class="text-center">
                    <?php if ($v["status"] == 3) {?>
                    <button type="button" class="btn btn-primary btn-bg" id="btn_approve" name="btn_approve"
                        onClick="show_criteria('<?=$v['occ_level_id']?>','<?=$v['occ_level_name']?>','<?=$v['place']?>','<?=$this->BaseModel->dateToThai($v['exam_date'], false)?>','<?=$v['exam_schedule_id']?>','<?=$v['status']?>','<?=$v['asm_tool_type']?>')">กำหนดเกณฑ์
                    </button>
                    <?php }if ($v["status"] == 5) {?>
                    <button type="button" class="btn btn-circle btn-success btn-xs mb-5"><i
                            class="fa fa-check"></i></button>

                    <?php }if ($v["status"] == 1 && $v["exam_template_id"] != "") {?>
...
<div class=" col-md-12" sytle="display:none">
    <?php
require_once dirname(__FILE__) . "../../../criteria_asm/form_approve_criteria.php";?>
</div>