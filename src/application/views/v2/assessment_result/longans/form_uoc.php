...
<div class="col-md-12">
    ...
    <div class="">
        ...
        <br />
        <?php if ($dataForEdit["evident"] != null) { ?>
        <div style="color:cornflowerblue">
            ไฟล์ปัจจุบัน :
            <?php
                $docFolder = "";
                if ($_POST['tool_type'] == '5') {
                    $docFolder = "demonstration_assessment_flie";
                } else if ($_POST['tool_type'] == '4') {
                    $docFolder = "assessment_detail_simulation_file";
                }
                foreach ($dataForEdit["evident"] as $f) {
                    $path = $docFolder . "/" . $_POST["app_id"] . "/";
                ?>
            <a style="color:cornflowerblue" href="<?= base_url() . $path .  $f->path_file ?>"
                target="_blank"><?= $f->name_file ?> </a><br />
            <?php }
